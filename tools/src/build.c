...
static const char *pick_checksum(Pkg *pkg, int i,
                                 const char **algo_out,
                                 const char **tool_out) {
    /* collect candidates from all three fields */
    ...
    /* pick strongest by detected length */
    const char *best = NULL;
    const char *best_algo = NULL, *best_tool = NULL;
    int best_strength = 0;  /* 3=sha512 2=sha256 1=md5 */

    for (int k = 0; k < nc; k++) {
        const char *a, *t;
        hash_detect(candidates[k], &a, &t);
        if (!a) continue;
        int strength = (a[3]=='5' && a[4]=='1') ? 3 :   /* sha512 */
                       (a[3]=='2')               ? 2 :   /* sha256 */
                                                   1;    /* md5    */
 ...
 */
/* ── URL builder: <REPO_BASE>/<repo>/<letter>/<pkgbuild_name> ────────── *
 * Layout: lotus-repository/main/<repo>/<a-z>/pkgbuild_<name>            *
 * The letter bucket is the first char of the package name, lowercased.   *
 * Non-alpha names fall into "0" bucket.                                  */
static void build_pkgbuild_url(char *out, size_t outsz,
                               const char *repo, const char *name) {
    char letter = name[0];
    if (letter >= 'A' && letter <= 'Z') letter += 32;
    if (letter < 'a' || letter > 'z')  letter = '0';
    snprintf(out, outsz, "%s/%s/%c/pkgbuild_%s",
             REPO_BASE, repo, letter, name);
}

static int pkg_exists_in_repo(const char *name) {
  for (int f = 0; f < NFOLDERS; f++) {
    char url[MAX_STR];
    build_pkgbuild_url(url, sizeof(url), FOLDERS[f], name);
...
static int fetch_pkgbuild(const char *name) {
  ...
  char url[MAX_STR];
  build_pkgbuild_url(url, sizeof(url), FOLDERS[folder], name);
  ...
  for (int qi = 0; qi < ndl; qi++) {
    ...
    if (fast_meta.is_binary) {
      ...
      for (int ri = 0; _bin_repos[ri] && !bin_ok; ri++) {
        ...
        printf(C_CYAN "(%d/%d)" C_RESET " Downloading [" C_BLUE "bin" C_RESET
               "] " C_BOLD "%s" C_RESET "...\n", qi + 1, ndl, pkgname);
      ...
      if (bin_ok) {
        printf(C_CYAN "(%d/%d)" C_RESET " Installing [" C_BLUE "bin" C_RESET
               "] " C_BOLD "%s" C_RESET "...\n", qi + 1, ndl, pkgname);
        lpkg_install_from_file(lpkg_dest);
      ...
      printf(C_YELLOW "warning:" C_RESET
             " binary for %s not found in repo, falling back to source build\n",
             pkgname);
    }
